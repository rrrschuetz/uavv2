...
def detect_and_label_blobs(image, num_detector_calls):
    ...
    red_contours = filter_contours(red_contours)
    ...
    if shared_race_mode.value == 3 and red_contours and green_contours and MONOCHROM_MODE:
        ...
        best_contour, best_label = max(
            all_contours,
            key=lambda cl: np.max(cl[0].reshape(-1, 2)[:, 1])
        )
        filtered_contours = [best_contour]
        print("best label", best_label)
    else:
        filtered_contours = red_contours + green_contours
        best_label = None  # will be determined per-contour

    x_coords = np.zeros(lab_image.shape[1], dtype=float)

    # Draw and label only selected blobs
    for box in filtered_contours:
        ...
        # Mark x_coords by color
        box_pts = box.reshape(-1, 2)
        left_end = np.min(box_pts[:, 0])
        right_end = np.max(box_pts[:, 0])
        x_coords[left_end:right_end] = 1.0 if label == 'R' else -1.0

        # Draw and label
        cv2.drawContours(image, [box], -1, (255, 255, 255), 3)
        cv2.putText(image, label, center, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        ...
        while not stop_event.is_set():
            ...
            if shared_race_mode.value in [0, 1, 3, 4]:
                ...
                if shared_race_mode.value == 1:
                    ...
                    if (not PARKING_MODE and abs(cum_heading) > 350 and Gfront_distance < 1.3) \
                        or (PARKING_MODE and abs(cum_heading) > 350 and num_laps < TOTAL_LAPS) \
                        or (PARKING_MODE and abs(cum_heading) > 290 and num_laps == TOTAL_LAPS - 1):
                        print(f"cum_heading {cum_heading} Gfront_distance {Gfront_distance}")
                    ...
                    #if PARKING_MODE and max_heading > 350 and abs(cum_heading) > 340:
                    if PARKING_MODE and abs(cum_heading) > 340:
                        shared_race_mode.value = 6
                        print("Pause 3 sec")
