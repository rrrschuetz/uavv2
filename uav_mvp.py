...
def camera_thread(picam0, picam1):
    global Gcolor_string, Gred_x_coords, Ggreen_x_coords
    ...
    while True:
        ...
        Gred_x_coords, Ggreen_x_coords, image = detect_and_label_blobs(cropped_image)
        end_time = time.time()

        # Convert arrays to lists of strings and join them into one string per array
        red_str = ",".join(map(str, Gred_x_coords.astype(int)))
        green_str = ",".join(map(str, Ggreen_x_coords.astype(int)))
