...
class mask():
    ...
    def red(self, image):
        lower_red = np.array([0, 140, 50])  #0,150,80  #0,130,0 #20.150.140
        upper_red = np.array([255, 255, 210]) #255,255,170 #255,255,255 #255,200,200
    ...
    def green(self, image):
        lower_green = np.array([0, 0, 50])  #0,0,120 #0,0,120 #20,0,0
    ...
    def magenta(self, image):
        lower_magenta = np.array([20, 150, 150])
...
def align_parallel(pca, sock, shared_race_mode, stop_distance=1.4, max_yaw_diff=10):
    ...
    while shared_race_mode.value == 2 and (distance2stop > 0 or distance2stop < -0.5):  # Distance left has prio
        ...
        print(
            f"front_distance: {front_distance:.2f} stop_distance: {stop_distance:.2f} distance2stop: {distance2stop:.2f} steer: {steer}")
...
def align_orthogonal(pca, sock, shared_race_mode, misalignment, stop_distance=0.05, max_yaw_diff=5):
    ...
    while shared_race_mode.value == 2 and abs(gyro_yaw_diff) > max_yaw_diff or distance2stop > 0:
        ...
        distance2stop = front_distance - stop_distance
        print(
            f"Gheading_start: {Gheading_start} Gheading_estimate: {Gheading_estimate} yaw_diff: {yaw_diff} gyro_yaw_diff: {gyro_yaw_diff}")
        print(
            f"front_distance: {front_distance:.2f} stop_distance: {stop_distance:.2f} distance2stop: {distance2stop:.2f} steer: {steer}")
        set_servo_angle(pca, 12, PARK_STEER * steer * SERVO_FACTOR + SERVO_BASIS)
        set_motor_speed(pca, 13, PARK_SPEED * MOTOR_FACTOR + MOTOR_BASIS)
...
def park(pca, sock, shared_race_mode, device):
    ...
    print(
        f">>> Car final heading: {Gheading_estimate} {orientation(Gheading_estimate) - orientation(Gheading_start):.2f}")
